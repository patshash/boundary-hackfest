...
resource "auth0_client" "my_client" {
  name                = var.hcp_boundary_cluster_id
  description         = "Boundary auth0 client application"
  app_type            = "regular_web"
  callbacks           = ["${hcp_boundary_cluster.this.cluster_url}/v1/auth-methods/oidc:authenticate:callback"]
  allowed_logout_urls = ["${hcp_boundary_cluster.this.cluster_url}"]
